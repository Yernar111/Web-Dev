...
@Component({
  ...
  `, // @defer block loads resource later. It has optional parameter (on viewport) which defers load once content enters the viewport(we see it on page). In this case, loads the "comments" component later 
  // @placeholder block shows some content before the deferred loading starts
  // @loading block shows some content while deferred loading runs. Both placeholder and loading have optional parameter (minimum) which prevents immediate loading/starting then starts it after some time  
  imports: [Comments],
})
export class App {}
